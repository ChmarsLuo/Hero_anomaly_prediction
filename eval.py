...
parser.add_argument('--show_curve', action='store_true', default=True,
    ...
    with torch.no_grad():
        for i, folder in enumerate(video_folders):
            ...
            for j, clip in enumerate(dataset):
                ...
                print('  test_pspr......',test_psnr)

                output = './output'
                if not os.path.exists(output):
                    os.makedirs(output)
                if test_psnr < 40:
                    target_np = (target_np + 1)*127.5
                    target_np = np.array(target_np)
                    target_np = target_np.transpose(2,1,0).astype('uint8')
                    print(target_np)
                    cv2.imwrite(f'./output/1.png', target_np)
